...
export default function MappingEditor() {
  ...
  const handleExecuteAll = async () => {
    ...
    await Promise.all(targets.map(async (target) => {
      ...
      try {
        const isDownload = !target.save_as_dataset && target.target_type !== "db";

        if (isDownload) {
          // Direkter File-Download via execute-download Endpunkt
          const resp = await api.post("/api/mappings/execute-download", {
            ...nodeCtx,
            targets: [target],
          }, { responseType: "blob" });
          if (abortRef.current) return;
          const ext = { xlsx: "xlsx", json: "json", xml: "xml" }[target.target_type] || "csv";
          const url = URL.createObjectURL(new Blob([resp.data]));
          const a = document.createElement("a"); a.href = url;
          a.download = `${target.name || "export"}.${ext}`; a.click();
