...
def _apply_join(left_df: pd.DataFrame, right_df: pd.DataFrame,
    ...
    # Anti-Join: Zeilen die in der anderen Tabelle NICHT vorkommen
    if join_type == "LEFT ANTI JOIN":
        try:
            merged = pd.merge(left_df, right_df[[right_key]], left_on=left_key, right_on=right_key, how="left", indicator=True, suffixes=("", "_r"))
            result = merged[merged["_merge"] == "left_only"].drop(columns=["_merge"])
            # Rechte Key-Spalte entfernen falls vorhanden
            if right_key in result.columns and right_key != left_key:
                result = result.drop(columns=[right_key])
            return result
        except Exception as e:
            raise ValueError(f"LEFT ANTI JOIN fehlgeschlagen: {e}")

    if join_type == "RIGHT ANTI JOIN":
        try:
            merged = pd.merge(left_df[[left_key]], right_df, left_on=left_key, right_on=right_key, how="right", indicator=True, suffixes=("_l", ""))
            result = merged[merged["_merge"] == "right_only"].drop(columns=["_merge"])
            if left_key in result.columns and left_key != right_key:
                result = result.drop(columns=[left_key])
            return result
        except Exception as e:
            raise ValueError(f"RIGHT ANTI JOIN fehlgeschlagen: {e}")
