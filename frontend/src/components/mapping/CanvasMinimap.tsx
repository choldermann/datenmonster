...
const NODE_COLORS = {
  ...
  expr:      "rgba(232,121,249,0.6)",
  quality:   "rgba(6,182,212,0.6)",
};

function buildNodes({ canvasNodes, transformNodes, constantNodes, sqlNodes, aggNodes, restNodes, lookupNodes, calcNodes, switchNodes, pythonNodes, aiNodes, exprNodes, qualityNodes }) {
  return [
    ...
    ...(aiNodes      || []).map(n => ({ x: n.x, y: n.y, w: 280, h: 300, c: NODE_COLORS.ai })),
    ...(exprNodes    || []).map(n => ({ x: n.x, y: n.y, w: 320, h: 180, c: NODE_COLORS.expr })),
    ...(qualityNodes || []).map(n => ({ x: n.x, y: n.y, w: 340, h: 200, c: NODE_COLORS.quality })),
  ];
}
...
export default function CanvasMinimap({
  ...
  aiNodes = [], exprNodes = [], qualityNodes = [],
  tick,
}) {
  ...
  const draw = useCallback(() => {
    ...
    const nodes = buildNodes({ canvasNodes, transformNodes, constantNodes, sqlNodes, aggNodes, restNodes, lookupNodes, calcNodes, switchNodes, pythonNodes, aiNodes, exprNodes, qualityNodes });
  ...
  }, [canvasRef, canvasNodes, transformNodes, constantNodes, sqlNodes, aggNodes, restNodes, lookupNodes, calcNodes, switchNodes, pythonNodes, aiNodes, exprNodes, qualityNodes]);
