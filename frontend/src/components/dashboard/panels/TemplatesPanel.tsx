  ...
  return (
    <div style={box}>
      ...
      {alle.map(t => {
        ...
        const p = t.preview || {};
        const c = p.counts || {};
        // c.knowledge muss mit rein: ein reines Wissensmodul hat weder Mappings
        // noch Reiter und wäre sonst gar nicht aufklappbar.
        const hatVorschau = (p.tabs?.length || p.requires?.length || p.hinweise?.length
                             || c.mappings || c.knowledge) > 0;
            ...
            <div style={{ marginTop: 10, marginLeft: 20, padding: "10px 12px", borderRadius: 6,
              ...
              {(c.mappings > 0 || c.forms > 0 || c.knowledge > 0) && (
                <VorschauZeile label="Umfang" text={[
                  ...
                  c.knowledge && `${c.knowledge} Wissensregel${c.knowledge > 1 ? "n" : ""}`,
                ].filter(Boolean).join(" · ")} />
              )}
              {p.knowledge?.length > 0 && (
                <VorschauZeile label="Wissen" text={p.knowledge.slice(0, 6).join(" · ")
                  + (p.knowledge.length > 6 ? ` … und ${p.knowledge.length - 6} weitere` : "")} />
              )}
