...
def run_mapping_object(
    ...
    # Projekt-Ausschlusslisten (Artikel und Kunden) in run_params injizieren
    # (idempotent – Formular-Pfade haben es ggf. schon gesetzt). Deckt
    # Pipeline/Scheduler-Läufe ab.
    if project_id is not None and db is not None:
        ...
        ctx.run_params = apply_customer_exclusions(ctx.run_params, project_id, db)
