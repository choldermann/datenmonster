...
# ─── Brute-Force-Schutz ───────────────────────────────────────────────────────
# In-Memory: {ip_or_username: [timestamp, timestamp, ...]}
...
def _check_rate_limit(key: str):
    """Prüft ob zu viele Login-Versuche für key (IP oder Username)."""
    now = time.time()
    with _attempts_lock:
        ...
        if len(attempts) >= MAX_ATTEMPTS:
            oldest = attempts[0]
            wait = int(LOCKOUT_SEC - (now - oldest))
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail=f"Zu viele Anmeldeversuche. Bitte {max(1, wait // 60)} Minute(n) warten.",
...
def login(
    ...
    if not user or not verify_password(form.password, user.hashed_password):
        ...
        _record_attempt(f"user:{form.username}")
        # Konstante Antwortzeit verhindert Username-Enumeration
        raise HTTPException(
