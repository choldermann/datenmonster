...
def get_monitoring(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    try:
        log_result = db.execute(
            text("SELECT id, level, module, action, message, entity_name, created_at, rows_processed, details, project_id FROM system_logs ORDER BY id DESC LIMIT 200")
        ).fetchall()
        for row in log_result:
            log_id, level, module, action, message, entity_name, created_at, rows, details_raw, log_project_id = row
            project_name = projects.get(log_project_id, "") if log_project_id else ""
            entry = {
                "id": log_id,
