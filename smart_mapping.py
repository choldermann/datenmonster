...
def _keyword_match(text: str, tables: list) -> list:
    ...
    Priorisiert exakte Treffer über Substring-Matches.
    """
...
def _fk_traversal(matched_tables: list, all_tables: list, relationships: list, max_tables: int = 10) -> list:
    """
    Findet via FK-Traversal verbundene Tabellen – nur Tiefe 1.
    Begrenzt auf max_tables Ergebnisse um Explosion bei großen Schemas zu vermeiden.
    """
    matched_keys = {t["key"] for t in matched_tables}
    result = list(matched_tables)

    # Nur direkt verbundene Tabellen (Tiefe 1) – kein transitives Traversal
    for rel in relationships:
        ...
        if from_in and not to_in:
            ...
            if t and t not in result:
                result.append(t)
                # matched_keys NICHT erweitern → kein transitives Traversal
